...
@section('content')
    ...
    <div class="card card-primary m-0 m-md-4 my-4 m-md-0 shadow">
        <div class="card-body">
            ...
            <form method="post" action="{{route('admin.planUpdate',[$data->id])}}" enctype="multipart/form-data" class="form-row justify-content-center">
                @csrf
                @method('put')
                    ...
                    <div class="row">
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select name="category_id" id="category_id" class="form-control">
                                <option value="" disabled>@lang('Select a Category')</option>
                                @foreach($categories as $item)
                                    <option value="{{$item->id}}" @if(old('category_id', $data->category_id) == $item->id) selected="selected" @endif>{{$item->name}}</option>
                                @endforeach
                            </select>
                            @error('category_id')
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                <label>@lang('Maximum Users')</label>
                                <input type="number" name="max_users" value="{{old('max_users', $data->max_users)}}" placeholder="@lang('Maximum Users')" class="form-control" >
                                @error('max_users')
                                <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                <label>@lang('Maximum Single User')</label>
                                <input type="number" name="max_per_user" value="{{old('max_per_user', $data->max_per_user)}}" placeholder="@lang('Maximum Single User')" class="form-control" >
                                @error('max_per_user')
                                <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                <label>@lang('Plan Thumbnail')</label>
                                <div class="image-input">
                                    <label for="image-upload" id="image-label"><i class="fas fa-upload"></i></label>
                                    <input type="file" name="image" placeholder="@lang('Choose image')" id="image">
                                    <img id="image_preview_container" class="preview-image" src="{{getFile(config('location.plan.path').$data->image) ? : 0}}"alt="preview image">
                                </div>
                                @error('image')
                                    <span class="text-danger">{{$message}}</span>
                                @enderror
