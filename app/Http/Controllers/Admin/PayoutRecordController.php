...
class PayoutRecordController extends Controller
{
    ...
    public function index()
    {
        ...
        $records = PayoutLog::where('status', '!=', 0)->orderBy('id', 'DESC')->with('user', 'method', 'account')->paginate(config('basic.paginate'));
    ...
    public function request()
    {
        ...
        $records = PayoutLog::where('status', 1)->orderBy('id', 'DESC')->with('user', 'method', 'account')->paginate(config('basic.paginate'));
        return view('admin.payout.logs', compact('records', 'page_title'));
    }
    ...
    public  function action(Request $request, $id){
        ...
        if($request->status == '2'){
            ...
            $this->sendMailSms($user, $type = 'PAYOUT_APPROVE', [
            ...
            $user = $data->user;
            $user[$data->balance_type] += ($data->amount + $data->charge);
            $user->save();

            $transaction = new Transaction();
            $transaction->user_id = $user->id;
            $transaction->amount = getAmount($data->amount + $data->charge);
            $transaction->charge = 0;
            $transaction->trx_type = '+';
            $transaction->balance_type = 'refund';
            $transaction->remarks = 'Refund Penarikan';
